# Continuum Soft Arm Controller (CSAC)

本项目用于三线绳驱连续体柔性机械臂的下位机控制。系统以 STM32F103C8T6 最小系统板为核心，完成机械臂进给、回撤、左右弯曲、上下弯曲、图案动作、急停保护、IMU 姿态采集和串口通信等低层实时任务。

机械臂采用三路电机驱动绳索执行机构，通过三电机速度混控实现进给与弯曲运动；同时结合 PS2 手柄和 Jetson/PC 串口命令，支持手动遥操作、上位机速度控制、状态查询和后续视觉/ROS 闭环扩展。

## 项目特点
- 三线绳驱连续体柔性机械臂控制，支持进给/回撤与二维弯曲。
- 三路 TB6612 电机驱动，TIM1 输出 PWM，TIM2/3/4 读取编码器速度反馈。
- PS2 手柄离线遥操作，支持摇杆控制、图案动作、使能、急停和夹爪控制。
- 三块十轴 IMU 姿态采集，当前默认传感器组合为 MPU6050 + HMC5883L + BMP180。
- USART3 与 Jetson/PC 通信，支持 ASCII 控制命令和 IMU/机械臂状态上报。
- 提供 Jetson/PC 侧 Python 工具，用于 IMU 实时显示、CSV 记录和基础自主靠近目标实验。

## 项目概览
- 控制对象: 三线绳驱连续体柔性机械臂
- 运动能力: 进给/回撤、左右弯曲、上下弯曲、预设图案动作
- MCU: STM32F103C8T6, 系统时钟 72MHz (SystemInit)
- Keil 工程: PS2_Demo.uvprojx
- 主入口: USER/main.c
- 核心功能: PS2 手柄控制 + 串口上位机控制 + 三电机闭环 + 三路 IMU 融合 + USART3 输出到 Jetson/PC
- 参考文档: `使用说明.md`, `技术说明.md`, `docs/control_protocol.md`, `docs/ps2_patterns.md`

## 硬件系统
| 模块 | 数量/型号 | 作用 |
| --- | --- | --- |
| 主控 | STM32F103C8T6 最小系统板 | 下位机实时控制、PWM、编码器、串口和 GPIO 管理 |
| 电机驱动 | 2 块 TB6612 驱动板 | 驱动三路线绳电机，完成进给和弯曲运动 |
| 遥操作 | PS2 手柄及接收器 | 手动控制、图案触发、使能、急停和夹爪控制 |
| 姿态传感 | 3 块十轴 IMU | 采集多段姿态，输出 roll/pitch/yaw、温度、气压和高度 |
| 通信 | USART3 串口 | 与 Jetson/PC 交换速度命令、状态帧和 IMU 数据 |
| 执行机构 | 三路绳驱电机/编码器 | 通过三电机混控实现连续体机械臂运动 |
